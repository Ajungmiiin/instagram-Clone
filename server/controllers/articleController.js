...
exports.find = async (req, res, next) => {
  try { // #README 1
    const where = {} // 검색조건
    const limit = req.query.limit || 9 //한 번에 전송할 도큐먼트의 갯수
    const skip = req.query.skip || 0 // 건너 뛸 도큐먼트 갯수
    ...
    // 게시물 검색
    const articleCount = await Article.count(where); // 해당 도큐먼트의 갯수를 구한다
    // Document
    const articles = await Article 
...
exports.delete = async(req, res, next) => {
  try {
    ...
    // 본인 게시물이 아닌 경우 삭제 불가
    // _id : ObjectId -> 객체끼리 비교불가, toString()을 이용해 문자열로 변환하여 비교
...
exports.unfavorite = async(req, res, next) => {
  try {
    ...
    if (favorite) { // 좋아요 한 게시물이면 좋아요 취소
